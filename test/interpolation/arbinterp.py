...
def calc_gamma_1d(x, fx, dfx, y, fpy, dfpy):
  ...
  isort = sorted(range(y.size), key=y.__getitem__)
  y, fpy, dfpy = y[isort], fpy[isort], dfpy[isort]
  # calculate derivative from fx
  xx = 0.5 * (x[1:] + x[:-1])
  fpx = (fx[1:] - fx[:-1]) / (x[1:] - x[:-1])
  dfpx = sqrt(dfx[1:]**2 + dfx[:-1]**2)
  # estimate beta*gamma
  ratio = ((dfpx**2).sum() + (dfpx**2).sum()) / \
          ((fpx**2).sum() + (fpy**2).sum())
...
def calc_gamma_bisect_1d(x, fx, dfx, y, fpy, dfpy):
  """
  Estimate the `wave number' parameter gamma from data points.
  This function prints stuff.
  """
...
def calc_gamma_nd_new(x, fx, dfx, y, fpy, dfpy):
  """
  Estimate the `wave number' parameter gamma from data points.
  This function prints stuff.
  """
